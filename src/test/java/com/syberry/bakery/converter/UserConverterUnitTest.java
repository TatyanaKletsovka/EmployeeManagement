...
public class UserConverterUnitTest {
    ...
    void should_SuccessfullyConvertToDtos() {
        when(roleConverter.convertToStringRoles(any()))
                .thenReturn(new ArrayList<>(List.of(RoleName.ROLE_USER.name())));
        User user = new User();
        user.setId(1L);
        user.setFirstName("test");
        user.setLastName("test");
        user.setEmail("test@mail.com");
        user.setRoles(new HashSet<>(List.of(new Role(1L, RoleName.ROLE_USER))));
        List<UserDto> resultDto = userConverter.convertToDtos(new ArrayList<>(List.of(user)));
        UserDto expectedDto = new UserDto(user.getId(), user.getFirstName(), user.getLastName(), user.getEmail(),
                new ArrayList<>(List.of(RoleName.ROLE_USER.name())));
    ...
    @Test
    void should_SuccessfullyConvertUserDtoToEntity() {
        when(roleConverter.convertToEntityRoles(any()))
                .thenReturn(new HashSet<>(List.of(new Role(1L, RoleName.ROLE_USER))));
        ...
        expectedUser.setRoles(new HashSet<>(List.of(new Role(1L, RoleName.ROLE_USER))));
        assertThat(resultUser).isEqualTo(expectedUser);
    }

    @Test
    void should_SuccessfullyConvertSignUpRequestDtoToEntity() {
        when(roleConverter.convertToEntityRoles(any()))
                .thenReturn(new HashSet<>(List.of(new Role(1L, RoleName.ROLE_USER))));
        UserDto user = new UserDto();
        user.setFirstName("test");
